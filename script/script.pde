...
void loop() {
  ...
  delay(1500);
  for(int i=hover+delta;i>=hover;i--) {
    spinA(i);
    spinB(i-yaw);
    delay(10);
    digitalWrite(13,(i/10) & 1);
  ...
  delay(5000);
  for(int i=hover;i>=hover-delta;i--) {
    spinA(i);
    spinB(i-yaw);
    delay(10);
    digitalWrite(13,(i/10) & 1);
...
void spinA(int spd) {
  analogWrite(3,spd>0?spd:0);
}

void spinB(int spd) {
  analogWrite(9,spd>0?spd:0);
}
